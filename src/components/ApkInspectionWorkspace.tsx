  ...
  return <section className="space-y-5">
    ...
    {inspection && <>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><div className="service-card p-6"><div className="flex items-center gap-2"><ShieldAlert className="text-[#934639]" size={18} /><div><p className="kicker text-[#934639]">{ar ? "الأذونات" : "Permissions"}</p><h3 className="mt-1 font-bold">{ar ? "راجع كل طلب قبل التثبيت." : "Review every request before installing."}</h3></div></div><div className="mt-5 space-y-2">{inspection.permissions.length ? inspection.permissions.map((permission) => { const sensitive = inspection.sensitivePermissions.includes(permission); return <article key={permission} className={`border-l-2 p-3 ${sensitive ? "border-[#c95a4b] bg-[#fbe5df] text-[#7d3028]" : "border-[#59869c] bg-[#f3efe6] text-[#526273]"}`}><div className="flex flex-wrap items-center gap-2"><span className="mono break-all text-[0.68rem] font-semibold">{permission}</span>{sensitive && <span className="status-stamp border-[#c95a4b] text-[#934639]">{ar ? "حساس" : "Sensitive"}</span>}</div><p className="mt-2 text-xs leading-5">{permissionExplanation(permission, ar)}</p></article>; }) : <p className="text-sm text-[#687584]">{ar ? "لم تُستخرج أذونات من البيان." : "No permissions were extracted from the manifest."}</p>}</div></div>
        <div className="service-card p-6"><div className="flex items-center gap-2"><Fingerprint className="text-[#59869c] dark:text-cyan-400" size={18} /><div><p className="kicker text-[#687584] dark:text-slate-400">{ar ? "سجل الناشر الموثوق محلياً" : "Local trusted publisher registry"}</p><h3 className="mt-1 font-bold dark:text-slate-100">{ar ? "قارن بصمة التوقيع قبل التثبيت." : "Compare a signing fingerprint before install."}</h3></div></div><p className="mt-3 text-xs leading-5 text-[#526273] dark:text-slate-300">{ar ? "هذه قائمة محلية في هذا المتصفح فقط. إضافة بصمة تعني أنك تثق بها بنفسك؛ لا تثبت أمان التطبيق أو مصدره وحدها." : "This is a browser-local list only. Adding a fingerprint records your own trust decision; it does not by itself prove an app is safe or authentic."}</p>{signerReferences.length ? <><div className="mt-4 space-y-2">{signerReferences.map((signer) => { const match = trustedMatch(signer.fingerprint); return <div key={signer.id} className={`border p-3 text-xs ${match ? "border-[#b9da71] bg-[#eef8cd] text-[#35501c] dark:border-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "border-[#d8d1c4] bg-[#fffdf8] text-[#526273] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"}`}><p className="font-semibold">{signer.scheme.toUpperCase()} · {match ? (ar ? `موثوق: ${match.label}` : `Trusted: ${match.label}`) : (ar ? "غير موجود في القائمة المحلية" : "Not in local registry")}</p><p className="mono mt-1 break-all text-[0.64rem]">{signer.fingerprint}</p>{signer.subject && <p className="mt-1 break-all">{signer.subject}</p>}</div>; })}</div><div className="mt-4 grid gap-2"><select value={trustCandidate} onChange={(event) => setTrustCandidate(event.target.value)} className="h-10 border border-[#d8d1c4] bg-[#fffdf8] px-3 text-xs outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-cyan-500 dark:focus:ring-1 dark:focus:ring-cyan-500"><option value="">{ar ? "اختر بصمة" : "Choose a fingerprint"}</option>{signerReferences.map((signer) => <option key={signer.id} value={signer.fingerprint}>{signer.scheme.toUpperCase()} · {signer.fingerprint.slice(0, 23)}…</option>)}</select><div className="flex gap-2"><input value={trustLabel} onChange={(event) => setTrustLabel(event.target.value)} placeholder={ar ? "اسم الناشر الموثوق" : "Trusted publisher name"} className="h-10 min-w-0 flex-1 border border-[#d8d1c4] bg-[#fffdf8] px-3 text-sm outline-none focus:border-[#59869c] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-cyan-500 dark:focus:ring-1 dark:focus:ring-cyan-500" /><Button onClick={addTrusted} disabled={!trustCandidate || !trustLabel.trim() || Boolean(trustedMatch(trustCandidate))} variant="outline" className="action-button border-[#59869c] text-[#35501c] dark:border-slate-700 dark:text-emerald-400"><Fingerprint className="mr-2" size={15} />{ar ? "حفظ" : "Trust"}</Button></div></div></> : <p className="mt-4 text-sm text-[#687584] dark:text-slate-400">{ar ? "لا توجد بصمة شهادة قابلة للمقارنة في هذا الملف." : "No certificate fingerprint could be compared for this file."}</p>}{trusted.length > 0 && <div className="mt-5 border-t border-[#d8d1c4] pt-4 dark:border-slate-800"><p className="kicker text-[#687584] dark:text-slate-400">{ar ? "محفوظ محلياً" : "Stored locally"}</p><div className="mt-2 space-y-2">{trusted.map((item) => <div key={item.id} className="flex gap-2 border border-[#d8d1c4] bg-[#fffdf8] p-2 dark:border-slate-700 dark:bg-slate-900"><div className="min-w-0 flex-1"><p className="text-xs font-semibold dark:text-slate-100">{item.label}</p><p className="mono mt-1 break-all text-[0.6rem] text-[#687584] dark:text-slate-400">{item.fingerprint}</p></div><button onClick={() => removeTrusted(item.id)} className="action-button self-start p-1 text-[#934639] dark:text-rose-400" aria-label={ar ? "حذف البصمة الموثوقة" : "Remove trusted fingerprint"}><Trash2 size={15} /></button></div>)}</div></div>}</div></div>
